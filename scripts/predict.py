...
INPUT_PATH = BASE_DIR / "data" / "predict_data.csv"
OUTPUT_PATH = BASE_DIR / "data" / "prediction_result.csv"
...
FEATURE_COLUMNS = [
    ...
    f"{i}_{col}" for i in range(1, 8) for col in [
        "選手名", "出身", "前級班", "現級班", "車番", "期別", "年齢",
        "競走得点", "逃", "捲", "差", "マ", "B", "H", "S",
        "勝率", "2連対率", "3連対率", "前走1", "前走2", "前走3", "今走1", "今走2"
    ]
]

CATEGORICAL_COLUMNS = ["競輪場"] + [
    f"{i}_{col}" for i in range(1, 8) for col in ["選手名", "出身"]
]
